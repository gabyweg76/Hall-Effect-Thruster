...
public:
	...
	double mass;
	double charge;
	double fi;      /*Current fraction of the species*/
...
int main()
{
	...
	ions.charge = 1 * QE;
	ions.mass = 131 * AMU;
	ions.spwt = 1e3;			/*GV: the specific weight is adapted according to our needs for the results*/
	...
	for (ts = 0; ts < 200; ts++)
	{
		...
		if ((ts + 1) % 1 == 0) Output(ts, &ions, ions.particles);
		if ((ts + 1) % 1 == 0) Output(ts, &ions2, ions2.particles);
...
{
	stringstream ss;
	ss << "results/particles_"<<species->name << setfill('0') <<setw(4) << ts+1 <<".csv";
	ofstream log(ss.str());

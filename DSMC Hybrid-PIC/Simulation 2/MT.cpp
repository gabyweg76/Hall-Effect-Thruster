...
const double PLASMA_DEN = 1e5;
const double NEUT_DEN = 1e6;
...
void OutputParticles(int ts, Species *species, vector<Particle>& particles);
...
void Output(int ts, Species *species, vector<Particle>& particles);
void Output2(int ts, Species *species, vector<Particle>& particles);
...
void MoveParticles(size_t p_start, size_t p_end, Species *species, Volume& volume, vector<Particle> &particles, FESolver& solver, double dt);
void Advance(int n_threads, Species *species, Volume& volume, vector<Particle> &particles, FESolver& solver, double dt);
...
int main(int argc, char *args[])
{
	...
	cout<<"Running with "<<n_threads<<" threads."<<endl;
	...
	for (int n = 0; n < n_nodes; n++)
	{
		...
		else if (volume.nodes[n].type == NEUMANN1) solver.g[n] = -10; /*phi_wall*/
		else if (volume.nodes[n].type == NEUMANN2) solver.g[n] = -10; /*phi_wall*/
    ...
    // stringstream ss;
	// ss << "results/particles.csv";
	// ofstream log(ss.str());
	// /*open log file if not yet open*/
    // if (!log.is_open()) {cerr<<"Failed to open file"<<ss.str()<<endl; exit(-1);}
	// log<<"ts,np_Xe+,np_Xe++,np_Xe, den_Xe+,den_Xe++,den_Xe,Mass,Momentum,Energy"<<"\n";

	/*main loop*/
	int ts;
	for (ts = 0; ts < 20000; ts++)
	{
		...
		/*Multhreading for Particle's motions and density*/
		Advance(num_threads, &ions, volume, ions.particles, solver, dt);
		Advance(num_threads, &ions2, volume, ions2.particles, solver, dt);
		Advance(num_threads, &ions3, volume, ions3.particles, solver, dt);
		...
		// if (MASS < tol && MOMENTUM < tol && ENERGY <tol) {
		// 		steady_state = true;
		...
		// if ((ts + 1) % 200 == 0) OutputMesh(ts, &ions, volume, solver.uh, solver.ef, ions.den);
		// if ((ts + 1) % 200 == 0) OutputMesh(ts, &ions2, volume, solver.uh, solver.ef, ions2.den);
		// if ((ts + 1) % 200 == 0) OutputMesh(ts, &ions3, volume, solver.uh, solver.ef, ions3.den);

        // /*output particles*/
        // if ((ts + 1) % 200 == 0) OutputParticles(ts, &ions, ions.particles);
        // if ((ts + 1) % 200 == 0) OutputParticles(ts, &ions2, ions2.particles);
        // if ((ts + 1) % 200 == 0) OutputParticles(ts, &ions3, ions3.particles);

		if ((ts + 1) % 400 == 0) Output(ts, &ions, ions.particles);
		if ((ts + 1) % 400 == 0) Output2(ts, &ions2, ions2.particles);

    
		// log <<ts<<","<<ions.particles.size()<<","<< ions2.particles.size() << "," << ions3.particles.size() << "," <<
		// max_den << "," << max_den2 << "," << max_den3 << "," << MASS << "," << MOMENTUM << "," << ENERGY << "," << endl;

        /*show some diagnostics*/
		cout<<"ts: "<<ts<<"\t Name:"<< ions.name <<"\t np:"<<ions.particles.size()<<"\t max den:"<<max_den<<endl;
		cout<<"ts: "<<ts<<"\t Name:"<< ions2.name <<"\t np:"<<ions2.particles.size()<<"\t max den:"<<max_den2<<endl;
        cout<<"ts: "<<ts<<"\t Name:"<< ions3.name <<"\t np:"<<ions3.particles.size()<<"\t max den:"<<max_den3<<endl;
...
{
	stringstream ss;
	ss << "ML/Xe+/particles_"<<species->name << setfill('0') <<setw(4) << ts+1 <<".csv";
	ofstream log(ss.str());
	/*open log file if not yet open*/
    if (!log.is_open()) {cerr<<"Failed to open file"<<ss.str()<<endl; exit(-1);}
	...
	double last_pos =0;
	/*write header*/
	log<<"pos(x),pos(y),pos(z),vel(x),vel(y),vel(z),Confined"<<"\n";
	for(int i=0; i<ts;i++) 
		for (Particle& part : particles)
		{		double current_pos = part.pos[0];
				double diff = current_pos - last_pos; 
				if (diff>0 && part.vel[0]> 0)
				{
					log<<part.pos[0] << "," << part.pos[1] << "," << part.pos[2] <<"," << part.vel[0]<< ","<< part.vel[1]<< ","<<part.vel[2]<<","<< "1" << "\n";
				}else 
				{
					log<<part.pos[0] << "," << part.pos[1] << "," << part.pos[2] <<"," << part.vel[0]<< ","<< part.vel[1]<< ","<<part.vel[2]<<","<< "0" << "\n";
				}
				last_pos = current_pos;
		}	
		
		if (ts%400==0) log.flush();
}
...
{
	stringstream ss;
	ss << "results/particles_" << species->name << setfill('0') << setw(4) << ts + 1 << ".vtp";
	ofstream out(ss.str());
 ...
 */
	...
	for (size_t p = p_start; p<p_end; p++)
	{
		...
		if (!alive){for(size_t p=0;p<np;p++)
		{particles[p] = particles[np-1]; np--; p--;}}
...
void Advance(int n_threads, Species *species, Volume& volume, vector<Particle> &particles, FESolver& solver, double dt)
{
	...
	for(int i=0;i<n_threads;i++){
		...
		threads.emplace_back(MoveParticles, p_start, p_end, ref(species), ref(volume), ref(particles), ref(solver), dt);
	}
	for (thread &t: threads) t.join(); //wait for threads to finish
